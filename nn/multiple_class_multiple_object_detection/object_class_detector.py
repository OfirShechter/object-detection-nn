...
class Object_Class_Detector():
    def __init__(self, model_path=None, iou_threshold=0.05, threshold=0.8, display_frames=True):
    ...
    def plot_marked_images(self, images):
        ...
        images_np = np.array([np.array(Image.fromarray(img))
                             for img in images_rgb])
        transformed_images = [self.transform(
            image=img)['image'] for img in images_np]
